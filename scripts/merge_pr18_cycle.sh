#!/usr/bin/env bash
# Option 3 cycle for PR 18: DELETE the reviews rule, merge, PUT it back.
# Pre-conditions: PR 18 is open and MERGEABLE; user is the repo admin.
# Post-conditions: PR 18 merged into main; protection restored from canonical file.
...
# CRITICAL: this cycle removes the 1-approval gate for a window. If any OTHER open
# PR exists, it could be merged during the window without a review. Refuse to run
# unless the only open PR is the target.
open_prs = json.load(open("open_prs.json"))
others = [pr for pr in open_prs if pr["number"] != 18]
if others:
    print(f"ABORT: {len(others)} other open PR(s) exist. The DELETE window would")
    print(f"unblock all of them. Close or merge them first, then re-run.")
    for pr in others:
        print(f"  - PR #{pr['number']}: {pr['title']}")
    sys.exit(1)

print("PRE-CONDITIONS OK: PR 18 is the only open PR, mergeable, blocked only by the reviews rule.")
PY

echo ""
echo "=== Step 1: DELETE the required_pull_request_reviews rule ==="
# Capture pre-DELETE state of the rule so we can verify restoration later.
...
echo ""
echo "=== Step 1a: confirm the rule is gone (expect 404) ==="
# Probe with the raw HTTP status. `gh api` swallows curl's write-out and we
# can't reliably get the code from it; use curl directly with the GH token.
...
if [ "$http_code" = "404" ]; then
    ...
    # GitHub's API quirk: after a DELETE, the endpoint may briefly return the
    # rule as a default-shape stub. Probe PR review state instead.
    echo "  -> 200 with default-shape stub. Verifying via PR review state..."
    pr_review_state=$(gh pr view "$PR" --json reviewDecision -q '.reviewDecision')
    if [ "$pr_review_state" = "" ] || [ "$pr_review_state" = "null" ]; then
        echo "  -> PR review state is empty/null: gate is effectively off. Proceeding."
    else
        echo "  -> PR review state: $pr_review_state. Aborting (gate is still on)."
        exit 1
    fi
else
    echo "  -> HTTP $http_code (expected 404 or 200-stub). Aborting."
    exit 1
fi
...
# Strip the _comment; the API doesn't accept it.
put_body = {
...
print("PUT body written to put_body.json")
PY

# Retry the PUT up to 3 times if it fails (network blips have hit this call before).
# A failed PUT leaves the branch unprotected, which is a real production incident,
# so we keep trying rather than aborting silently.
put_attempt=1
put_max=3
while [ $put_attempt -le $put_max ]; do
    ...
    echo "  -> PUT attempt $put_attempt failed. Retrying in $((put_attempt * 3))s..."
    sleep $((put_attempt * 3))
    put_attempt=$((put_attempt + 1))
done
if [ $put_attempt -gt $put_max ]; then
    echo "ERROR: PUT failed after $put_max attempts. The branch is currently UNPROTECTED."
    echo "Manual recovery: gh api -X PUT repos/$REPO/branches/main/protection --input put_body.json"
    exit 1
fi

echo ""
echo "=== Step 3a: confirm the rule is restored ==="
...
if pre == post:
    print("OK: protection rule restored to pre-cycle state (every policy value matches).")
else:
...
echo ""
echo "=== Step 4: capture post-merge audit ==="
gh pr view "$PR" --json state,mergeCommit,mergedAt,mergedBy > "post_pr.json"
python - <<PY
import json
audit = {
    "pr": $PR,
    "method": "Option 3 cycle: DELETE reviews rule -> MERGE --squash -> PUT reviews rule back",
    "pre_pr": json.load(open("pre_pr.json")),
