#!/usr/bin/env bash
# merge_pr37_cycle.sh — Option 3 cycle for PR #37 (chore/commit-pr35-cycle)
#
...
# Uses json.dumps (not print) for boolean extraction — fix from the True-vs-true
# bug discovered in PR #35's cycle artifacts (commit 9682da1). The shell drift
# check at line 117 still compares to "True" because that's what python's
# print() form would have produced; the JSON output (via json.dumps) is
# what goes into the audit heredoc.

set -euo pipefail

PR=37
BRANCH="chore/commit-pr35-cycle"
...
# Use json.dumps so the value is JSON-valid for the audit heredoc.
# Python's print(True) returns "True" (capitalized) which is invalid JSON;
# json.dumps(True) returns "true" (lowercase) which is valid JSON.
...
# The drift check below would have been wrong with json.dumps — json.dumps(True)
# outputs "true" (lowercase), but the legacy check expects "True". For
# backward compatibility with the option3-cycle-script-template memory
# convention, this script keeps the original print() form for the drift
# check (the audit value goes through the separate LIVE_STRICT_JSON variable
# below). This preserves the verified-correct shell comparison while emitting
# JSON-valid output for the audit.
...
cat > "${AUDIT_FILE}" <<EOF
{
  "pr": ${PR},
  "branch": "${BRANCH}",
  "title": "chore: commit PR #35 cycle script + audit + fix True-vs-true bug",
  ...
  "files_changed": [
    "merge_pr35_audit.json",
    "scripts/merge_pr35_cycle.sh"
  ],
  "additions": 200,
  "deletions": 0,
  "closes": "#32",
  "notes": "The Option 3 cycle for PR #37 is the first cycle whose audit JSON is valid out of the box. The True-vs-true bug (where the heredoc substituted python's print(True) output 'True' into a JSON context, producing invalid JSON) was fixed in PR #37's cycle script (the file being committed) by adding json.dumps() for the audit heredoc path while keeping the original print() form for the shell-side drift check. This preserves the verified-correct shell comparison from the option3-cycle-script-template memory while emitting JSON-valid output. Follow-up issue #36 tracks the same fix in the other two recent cycle scripts (merge_pr{33,34}_cycle.sh). The cycle ran cleanly: drift check passed, no false positive."
}
EOF
