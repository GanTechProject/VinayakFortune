#!/usr/bin/env bash
# Option 3 cycle for PR 27: DELETE the reviews rule, merge, PUT it back.
# Pre-conditions: PR 27 is open and MERGEABLE; user is the repo admin.
# Post-conditions: PR 27 merged into main; protection restored from canonical file.
...
others = [pr for pr in open_prs if pr["number"] != 27]
if others:
...
print("PRE-CONDITIONS OK: PR 27 is the only open PR, mergeable, blocked only by the reviews rule.")
PY
